...
class QueueEndpointsSmokeTests(TestCase):
    ...
    def test_nurse_queue_patients_shape(self):
        resp = self.client.get("/api/operations/nurse/queue/patients/?department=OPD")
    ...
    def test_create_and_list_queue_schedule(self):
        ...
        create_resp = self.client.post("/api/operations/queue/schedules/", create_payload, format="json")
        self.assertEqual(create_resp.status_code, 201)
        created = create_resp.json()
        self.assertEqual(created["department"], "OPD")
        list_resp = self.client.get("/api/operations/queue/schedules/")
    ...
    def test_toggle_queue_status(self):
        ...
        resp = self.client.post("/api/operations/queue/status/", payload, format="json")
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(resp.json().get("is_open"))
        get_resp = self.client.get("/api/operations/queue/status/?department=OPD")
