...
class PatientSymptomsEndpointTests(TestCase):
    def setUp(self):
        self.patient_user = User.objects.create_user(
            email="symptoms-patient@example.com",
            password="Testpass123",
            full_name="Symptoms Patient",
            role=User.Role.PATIENT,
        )
        self.patient_profile = PatientProfile.objects.create(user=self.patient_user)
        self.client = APIClient()
    ...
    def test_patient_symptoms_get_and_put(self):
        get_resp = self.client.get("/users/patient/symptoms/")
        self.assertEqual(get_resp.status_code, 200)
        payload = get_resp.json() or {}
        self.assertTrue(payload.get("success"))
        self.assertEqual((payload.get("data") or {}).get("symptoms"), [])

        put_resp = self.client.put("/users/patient/symptoms/", {"symptoms": "Fever and headache"}, format="json")
        self.assertEqual(put_resp.status_code, 200)
        put_payload = put_resp.json() or {}
        self.assertTrue(put_payload.get("success"))
        data = put_payload.get("data") or {}
        self.assertTrue(isinstance(data.get("symptoms"), list))
        self.assertTrue(any((i or {}).get("text") == "Fever and headache" for i in data.get("symptoms") or []))

        get_resp2 = self.client.get("/users/patient/symptoms/")
        self.assertEqual(get_resp2.status_code, 200)
        payload2 = get_resp2.json() or {}
        data2 = payload2.get("data") or {}
        self.assertTrue(isinstance(data2.get("symptoms"), list))
        self.assertTrue(any((i or {}).get("text") == "Fever and headache" for i in data2.get("symptoms") or []))
