...
def risk_assessment_audit(request):
    ...
    if RiskAssessmentAuditLog is None:
        return Response(
            {"success": True, "message": "Audit log unavailable.", "data": {"events": []}},
            status=status.HTTP_200_OK,
        )

    qs = RiskAssessmentAuditLog.objects.all().order_by("-created_at")[:limit]
    events = []
    for row in qs:
        events.append(
            {
    ...
    return Response(
        {"success": True, "message": "Audit events retrieved successfully", "data": {"events": events}},
        status=status.HTTP_200_OK,
    )
